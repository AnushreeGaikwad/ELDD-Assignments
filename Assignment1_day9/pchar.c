/*Q.1. In pchar driver, use semaphore to ensure that only one process can perfrom IO operations on device. If device is opened by one process, it should not be
accessible to another process  */
...
static int pchar_open(struct inode *pinode, struct file *pfile) {
    ...
    pdev = container_of(pinode->i_cdev, struct pchar_device, cdev);
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("Anushree Gaikwad");
MODULE_DESCRIPTION("semaphore assignment");
